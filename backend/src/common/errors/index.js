...
const Errors = {
  ...
  // 验证错误
  validation: (detail) => new AppError(ErrorCodes.VALIDATION_FAILED, typeof detail === 'string' ? detail : JSON.stringify(detail)),
  missingParam: (param) => new AppError(ErrorCodes.MISSING_PARAM, `缺少必需参数: ${param}`),
  invalidParam: (param, reason) =>
  ...
  // 限流错误
  rateLimit: (message) => AppError.rateLimit(message),
  ...
  // 工具错误
  toolNotFound: (tool) => new AppError(ErrorCodes.TOOL_NOT_FOUND, `工具不存在: ${tool}`),
  toolExecError: (tool, reason) =>
    new AppError(ErrorCodes.TOOL_EXEC_FAILED, reason ? `工具 ${tool} 执行失败: ${reason}` : `工具 ${tool} 执行失败`),
  toolTimeout: (tool) => new AppError(ErrorCodes.TOOL_TIMEOUT, `工具 ${tool} 执行超时`),
};
