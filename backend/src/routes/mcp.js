...
router.post('/tools/:name/test', async (req, res) => {
  ...
  catch (error) { res.status(500).json({ success: false, error: { code: 'TEST_FAILED', message: error.message } }); }
});
...
router.post('/call', async (req, res) => {
  ...
  } catch (error) { res.status(500).json({ success: false, error: { code: 'EXECUTION_FAILED', message: error.message } }); }
});

router.get('/stats', (_req, res) => { try { res.json({ success: true, stats: mcpService.getExecutionStats() }); } catch (error) { res.status(500).json({ success: false, error: error.message }); } });
