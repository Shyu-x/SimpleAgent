...
const { ReActAgent, PlanExecuteAgent, CodeActAgent, Text2SQLAgent, AgentFactory, AgentType } = require('../services/multiAgentEngine');
const { ExtendedToolRegistry } = require('../services/extendedTools');

const toolRegistry = new ExtendedToolRegistry();
const sessions = new Map(); // 临时会话存储，后续可迁移到服务
...
// POST /sessions/:sessionId/run - 运行Agent (SSE流式)
...
// POST /sessions/:sessionId/pause - 暂停执行
...
// POST /sessions/:sessionId/resume - 恢复执行
...
// DELETE /sessions/:sessionId - 终止会话
...
// POST /text2sql/config - 配置Text2SQL
router.post('/text2sql/config', (req, res) => {
  try {
    const { sessionId, dbConfig } = req.body;
    const agent = sessions.get(sessionId);
    if (!agent || agent.name !== 'Text2SQL Agent') {
      return res.status(400).json({ error: { message: 'Invalid session or not a Text2SQL agent', type: 'validation_error' } });
    }
